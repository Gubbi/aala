...
This folder drills into each container from [L2](../L2/) and describes the components inside. For every container in L2, there is a corresponding chapter here, plus a chapter for the cross-cutting projection facet. The chapters are numbered to align with L2 (e.g., `03-atoms.md` here describes the components of the container documented in `L2/03-atoms.md`).
...
Projection is a cross-cutting read facet rather than a container; its component-level shape is documented in [`04-projection.md`](./04-projection.md) and is implemented by each contentful container over its own content.
...
The canonical data model (the `Atom` type hierarchy and its supporting types) lives in [`03-atoms.md`](./03-atoms.md) since Atoms owns the schema. Other chapters cross-reference it.

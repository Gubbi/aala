...
Ingestion is **tree-agnostic**. Tree assignment for extracted atoms happens downstream in Atoms (via extractor hints, ingest options, or default tree configuration). Ingestion preserves the fragment's source semantics without committing to a tree.
...
- **Raw Store** — durable, append-only persistence layer (implementation-specific backend).
- **Change Log** — maintains the ordered, append-only event log for this container. Receives `FragmentAccepted` / `FragmentRejected` notifications. Serves `changes_since(ref)` for any downstream consumer that wants to react to new fragments.

Full L3 detail lives in [`docs/L3/02-ingestion.md`](../L3/02-ingestion.md).
...
## Relationship to fragment context
